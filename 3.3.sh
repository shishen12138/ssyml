...
# ---------------- 停止旧服务 ----------------
echo "停止旧服务..."
for svc in miner.service cpu-watchdog.service agent.service; do
...
if [ -f /etc/debian_version ]; then
    apt update
    apt install -y wget build-essential git python3-venv python3-pip python3-wheel python3-minimal python3-setuptools || true
    # 修复破损包
    dpkg --configure -a || true
    apt install -f -y || true
elif [ -f /etc/redhat-release ]; then
    yum install -y wget gcc gcc-c++ make git python3-venv python3-pip || true
else
    echo "未知 Linux 发行版"
    exit 1
fi
...
WATCHDOG_PATH="/etc/systemd/system/cpu-watchdog.service"
...
# ---------------- 创建 agent.service ----------------
AGENT_PATH="/etc/systemd/system/agent.service"
