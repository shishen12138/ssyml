...
install_python_packages() {
    echo "安装 Python 库..."
    PIP_PACKAGES=("flask" "flask-socketio" "asyncssh" "boto3" "requests" "psutil" "eventlet")

    # Debian 系统优先用 apt 安装部分库
    if [ "$OS" == "debian" ]; then
        sudo apt install -y python3-flask python3-requests python3-psutil python3-boto3 python3-blinker python3-cryptography || true
    fi

    # 强制覆盖安装
    pip3 install --upgrade --force-reinstall pip --break-system-packages
    pip3 install --upgrade --force-reinstall "${PIP_PACKAGES[@]}" --break-system-packages
}
